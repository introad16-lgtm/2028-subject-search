...
MAJOR_DB = {
    ...
    "영어영문학과": {
        "topics": ["인공지능 번역기의 발달이 영문학 번역의 질에 미치는 영향 분석", "영미권 문학 작품에 나타난 소수자 인권 문제 비교 연구", "글로벌 비즈니스 환경에서의 효과적인 영문 커뮤니케이션 전략"],
        "books": ["오만과 편견", "영어의 역사", "위대한 개츠비"], "links": ["한국영어영문학회", "British Council"]
    },
    "심리학과": {
        "topics": ["확증 편향이 디지털 알고리즘과 만났을 때 생기는 사회적 문제와 대안", "청소년 우울증 예방을 위한 학교 내 인지행동치료(CBT) 적용 방안", "범죄 심리학 관점에서 본 사이버 폭력 가해자의 심리 기제"],
        "books": ["스키너의 심리상자 열기", "아내를 모자로 착각한 남자", "생각에 관한 생각"], "links": ["한국심리학회", "마인드포스트"]
    },
    "경영학과": {
        "topics": ["ESG 경영 도입이 기업의 장기적 가치에 미치는 실증적 분석", "플랫폼 비즈니스 모델의 시장 독점 문제와 벤처 생태계 조성 방안", "행동경제학(넛지)을 적용한 긍정적 마케팅 전략 사례 연구"],
    ...
    "인공지능(AI)학과": {
        "topics": ["생성형 AI(LLM)의 할루시네이션(환각) 현상 원인 및 완화 알고리즘 탐구", "머신러닝 알고리즘에 내재된 데이터 편향성 문제와 공정성 확보 방안", "의료 영상 데이터를 활용한 딥러닝 기반 질병 조기 진단 모델 연구"],
...
TRACK_DB = {
    ...
    "사회계열": {"topics": ["현대 사회의 플랫폼 독과점이 시장 경제에 미치는 영향 고찰", "디지털 소외 계층을 위한 보편적 복지 정책 제안", "저출산 고령화 사회의 사회 구조적 원인과 제도적 대안"], "books": ["넛지", "총, 균, 쇠"], "links": ["통계청", "국회예산정책처"]},
    ...
    "공학계열": {"topics": ["지속 가능한 발전을 위한 친환경 에너지 저장 시스템(ESS) 분석", "스마트 시티 구축을 위한 사물인터넷(IoT) 기술의 활용", "웨어러블 로봇 기술이 산업 현장에 미치는 인간공학적 분석"], "books": ["거의 모든 IT의 역사", "공학의 눈으로 미래를 설계하라"], "links": ["KIST 한국과학기술연구원", "국가과학기술지식정보서비스"]},
...
def show_offline_result(track, target_name, selected_act, custom_title, act_type):
    data = MAJOR_DB.get(target_name, TRACK_DB.get(track, TRACK_DB["인문계열"]))
    custom_text = custom_title if custom_title else "관련 심화 주제"
    
    # 💡 활동 유형에 따른 차별화된 조언 생성
    if act_type == "주도형":
        act_advice = f"선택하신 **[{selected_act}]**은 학생 스스로 기획하고 탐구하는 **학생 주도형 활동**입니다. 위의 주제 중 하나를 선택해 본인만의 가설을 세우고, 자료 조사부터 결론 도출까지의 자기주도적 역량을 생기부에 구체적으로 녹여내세요."
    else:
        act_advice = f"선택하신 **[{selected_act}]**은 강연 청취 및 실습 중심의 **비주도형 활동**입니다. 수동적인 참여 소감에 그치지 말고, 배운 내용을 바탕으로 위 주제와 연결하여 **'추가 문헌 조사 및 심층 소논문 작성'** 등 후속 탐구를 전개하여 전공에 대한 주도성을 증명하세요."

    offline_md = f"""
    ### 💡 1. [{target_name}] 맞춤형 심층 탐구 주제 제안 (학교 DB)
    * **[주제 1]** {data['topics'][0]}
    * **[주제 2]** {data['topics'][1]}
    * **[주제 3]** {data['topics'][2]}
    ...
    * **[추천 도서]** 『{data['books'][0]}』, 『{data['books'][1]}』 외 {target_name} 분야의 융합적 시각을 다룬 대학 교양 수준의 입문서 자율 탐독
    ...
    * 진로와 연관된 **국가 연구소 (예: KDI, KIST, STEPI 등)** 홈페이지의 최신 연구 동향(Press Release) 확인 및 {data['links'][0]}, {data['links'][1]} 참조
    """
    
    # 선생님이 원하시던 디자인 100% 일치 유지
    st.markdown(f"""
...
career_data = {
    ...
    "사회계열": ["계열 전반 (특정 학과 미정)", "경영학과", "경제학과", "정치외교학과", "사회복지학과", "미디어커뮤니케이션학과", "행정학과", "국제통상학과", "회계학과", "관광학과", "사회학과"],
    "교육계열": ["계열 전반 (특정 학과 미정)", "초등교육과", "국어교육과", "영어교육과", "수학교육과", "역사교육과", "유아교육과", "특수교육과", "교육학과"],
    "공학계열": ["계열 전반 (특정 학과 미정)", "컴퓨터공학과", "인공지능(AI)학과", "기계공학과", "전기전자공학과", "화학공학과", "신소재공학과", "반도체공학과", "산업공학과", "소프트웨어공학과", "건축공학과"],
    "자연계열": ["계열 전반 (특정 학과 미정)", "수학과", "물리학과", "화학과", "생명과학과", "환경과학과", "지구환경과학과", "해양학과", "통계학과", "천문우주학과"],
    "의약계열": ["계열 전반 (특정 학과 미정)", "의예과", "치의예과", "한의예과", "약학과", "간호학과", "수의예과", "보건행정학과", "물리치료학과", "임상병리학과"],
    "예체능계열": ["계열 전반 (특정 학과 미정)", "디자인학과", "시각디자인학과", "산업디자인학과", "패션디자인학과", "회화과", "체육학과", "연극영화과", "무용과", "스포츠산업학과"]
}
    ...
    if act_type == "비주도형" and not custom_title: 
        ...
        if "학교 자체" in engine_choice:
            ...
            if not api_key:
                ...
                양식:
                ### 💡 1. [{target_name}] 맞춤형 심층 탐구 주제 제안 (최소 3가지 이상 상세히)
                ### 🎯 2. 탐구를 위한 구체적인 활동 전개 팁 ({act_type}의 특성을 반영하여 주도성 및 후속 활동 강조)
