...
if gemini_btn:
    if not api_key: st.error("🚨 제미나이 API 키가 시스템에 설정되어 있지 않습니다. 선생님께 문의하세요!")
        ...
        if act_type == "주도형":
            ...
            ### 📖 3. 심화 탐구를 위한 추천 참고 문헌 (추천 도서 2권 및 RISS/KCI 논문 검색 키워드)
            ### 🔗 4. [{target_name}] 전공 탐색 추천 웹사이트 (주요 대학 학과 홈페이지, 관련 국책 연구소, 학회 링크 등)
            """
        else:
            prompt = f"""
            당신은 대한민국 최고 수준의 고등학교 진로진학 전문 교사입니다.
            ...
            ### 📖 3. 심화 탐구를 위한 추천 참고 문헌 (강의 내용과 전공을 엮을 수 있는 도서 2권 및 논문 키워드)
            ### 🔗 4. [{target_name}] 전공 탐색 추천 웹사이트 (주요 대학 학과 홈페이지, 관련 국책 연구소, 학회 등)
            """
