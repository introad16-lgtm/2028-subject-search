...
# ----------------- 3학년 (수시 6장 신규: 70% 보수적 컷 + AI 융합) 프롬프트 -----------------
PROMPT_3_6CARDS = f"""
...
[AI 자율 학과/대학 추천 알고리즘 (수직/수평 이동 전략)]
1. 수직 이동 (점수 기반 상/하향): 학생의 내신이 희망 전공 합격선보다 압도적으로 높다면, 스스로 판단하여 동일 계열 내 최상위 티어를 상향 1순위 카드로 자동 격상하여 추천하십시오. 반대로 성적이 모자란다면, 무리한 상향 대신 하위 티어 전공이나 한 단계 낮은 대학으로 하향하여 추천하십시오.
2. 수평 이동 (생기부 기반 틈새 전공): 학생의 성적이 메인 희망 학과를 뚫기에 부족할 경우, 생기부 텍스트 내의 핵심 탐구 주제와 교집합이 있는 '유사/틈새 학과'를 스스로 유추하여 우회 전략으로 추천하십시오.

[수시 6장 포트폴리오 설계 절대 원칙 (현실성 및 70% 컷 필터)]
1. 현실적 상향 한계선 (Max +0.5~1.0): '상향(도전)' 카드를 구성할 때, 학생의 현재 내신 등급에서 최대 0.5~1.0 등급 이내의 대학/학과만 추천하십시오. (예: 6등급대 학생에게 2~3등급대 대학 추천 절대 금지)
2. 양명여고 엑셀 통계 최우선 반영: 제공된 [2. 양명여고 합불 통계]의 실제 합격/불합격 내신(평균내신 등)을 가장 최우선 기준으로 삼으십시오.
3. 내신 대역별 권역 가이드라인 엄수: (1~2등급: 인서울 주요 / 3~4등급: 인서울 중하위 및 수도권 주요, 지거국 / 5~6등급: 수도권 외곽, 지방 소재 4년제, 전문대학 등). 학생의 내신이 낮다면 상향이라 하더라도 지방 4년제 상위권이나 수도권 전문대 정도로 제한하여 희망 고문을 원천 차단하십시오.

[70% 예상컷 보수적 산출 원칙 (신규 필수 준수)]
* 데이터 보정 기준: 대학별 공식 발표 입시결과(어디가 70% cut 등)를 베이스로 하되, 특목고/자사고 표본이 혼재된 점을 감안하여 일반계 고등학교 학생 기준의 실질 합격선으로 보정할 것.
* 보수적 타겟팅: 수시 지원의 안정성을 최우선으로 고려하여, 서류의 정성적 가점을 배제하고 오직 '일반고 내신' 기준으로 실제 발표 컷보다 0.2~0.3등급 타이트하게 상향 조정한 '보수적 70% 예상컷'을 도출하여 표에 명시할 것.
...
| 지원 전략 | 추천 대학 및 세부 전형명 (교과/학종/논술 포함) | 지원 학과(전공) | 보수적 70% 예상컷 | 추천 사유 및 합격 가능성 분석 (개조식) |
| :--- | :--- | :--- | :--- | :--- |
| **상향 (도전) 1** | (예: OO대학교 OO전형) | (예: 미디어커뮤니케이션학과) | (예: 2.3~2.5) | * 내용 |
| **상향 (도전) 2** | | | | * 내용 |
| **적정 1** | | | | * 내용 |
| **적정 2** | | | | * 내용 |
| **안정 1** | | | | * 내용 |
...
| 지원 전략 | 추천 대학 및 세부 전형명 (교과/학종/논술 포함) | 지원 학과(전공) | 보수적 70% 예상컷 | 추천 사유 및 합격 가능성 분석 (개조식) |
| :--- | :--- | :--- | :--- | :--- |
| **상향 (도전) 1** | (예: OO대학교 OO전형) | (예: 경영학과) | | * 내용 |
| **상향 (도전) 2** | | | | * 내용 |
| **적정 1** | | | | * 내용 |
| **적정 2** | | | | * 내용 |
| **안정 1** | | | | * 내용 |
...
| 지원 전략 | 대학교 및 세부 전형명 | 지원 학과(전공) | 보수적 70% 예상컷 | 최적의 조합 선정 사유 및 면접 승부처 (개조식) |
| :--- | :--- | :--- | :--- | :--- |
...
### 6. 📊 보수적 70% 예상컷 산출 근거
(제시된 보수적 70% 예상컷의 신뢰도를 높이기 위해, 어떤 기준과 알고리즘으로 도출했는지 다음 요소를 포함하여 3~4줄로 명확하게 밝혀 작성할 것: 대학별 공식 '어디가' 발표 자료 베이스, 일반고 표본 기준 내신 디플레이션(0.2~0.3등급 상향) 적용, 학생의 개별 성적 추이(우상향 등) 정성 평가 반영 여부 등)
