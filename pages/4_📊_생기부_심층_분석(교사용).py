...
1. 표를 생성할 때 각 행(Row)은 반드시 엔터(줄바꿈)로 구분하여 정상적인 Markdown 표 형식을 유지하십시오. 전체 내용을 한 줄로 이어 쓰지 마십시오!
2. 표의 '셀(Cell) 내부' 텍스트 안에서만 줄바꿈을 위해 `<br>` 태그를 사용하십시오. (셀 안에서만 엔터 금지)
...
6. 핵심 상담 포인트
* 🎯 **[원포인트 지원 전략]**: (예시: 성신여대 지원 시, 내신 컷이 높은 교과 전형보다는 전공 적합성을 높게 평가하는 학종 전형으로 우회하는 합격 확률을 40% 이상 높일 수 있음. 구체적인 비교 우위 전략 1문장 필수)
