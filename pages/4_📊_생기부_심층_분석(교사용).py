...
# 🔥 환산 내신 절대 준수 규칙 및 예외 처리 (1, 2학년 전용 - 2학년 에러 방지 핵심!)
GRADE_CONVERT_PROMPT = """
[🚨 내신 환산 절대 규칙 및 데이터 예외 처리 (환각 방지)]
1. 내신 점수, 희망 대학 등 사전 입력 정보가 비어있더라도 [🚨 실제 생기부 텍스트]가 존재하면 절대 "데이터가 없습니다"라며 멈추지 마십시오. 주어진 생기부 내용만으로 표를 정상적으로 작성하십시오.
2. 학생 데이터에 '양명여고 산출기 기준 9등급제 환산' 점수가 주어지면, AI는 절대 임의로 5등급 백분위를 추정하지 마십시오. 반드시 제공된 9등급제 환산 점수를 100% 신뢰하여 과거 입결 데이터 비교 및 대학 라인 산정의 '절대 기준점'으로 삼으십시오.
"""
...
# ----------------- 2학년 프롬프트 -----------------
# 💡 2학년 표 양식을 3학년과 100% 동일하게 맞춰서 텍스트 깨짐 및 누락 에러 원천 차단!
PROMPT_2 = f"""
...
# ----------------- 3학년 프롬프트 -----------------
# 💡 선생님의 지시대로 3학년 프롬프트는 단 한 글자도 건드리지 않았습니다!
PROMPT_3 = f"""
