...
1. 수직 이동 (점수 기반 상/하향): 학생의 내신이 희망 전공(예: 지방 약대) 합격선보다 압도적으로 높다면(예: 1.0), 스스로 판단하여 동일 계열 내 최상위 티어(예: 인서울 메이저 의대)를 상향 1순위 카드로 자동 격상하여 추천하십시오. 반대로 성적이 모자란다면, 무리한 상향 대신 하위 티어 전공이나 한 단계 낮은 대학으로 하향하여 추천하십시오.
2. 수평 이동 (생기부 기반 틈새 전공): 학생의 성적이 메인 희망 학과를 뚫기에 부족할 경우, 생기부 텍스트 내의 핵심 탐구 주제와 교집합이 있는 '유사/틈새 학과'를 스스로 유추하여 우회 전략으로 추천하십시오. (예: '컴퓨터공학' 지망이나 생명과학 세특이 좋다면 '생물정보학(바이오인포매틱스)', '기계공학' 지망이나 자동차 탐구가 있다면 '미래자동차학과' 등)
...
3. 보수적 적정 라인 (70% 컷 기준): 일반적인 대학 입시 결과를 참조할 경우, 문 닫고 들어가는 기형적인 컷트라인이 아니라 '해당 학과의 70% 컷(100명 중 70등 합격 점수)'을 적정 지원의 마지노선으로 삼아 보수적으로 대학 라인을 설정하십시오.
4. 내신 대역별 권역 가이드라인 엄수: (1~2등급: 인서울 주요 / 3~4등급: 인서울 중하위 및 수도권 주요, 지거국 / 5~6등급: 수도권 외곽, 지방 소재 4년제, 전문대학 등). 학생의 내신이 낮다면 상향이라 하더라도 지방 4년제 상위권이나 수도권 전문대 정도로 제한하여 희망 고문을 원천 차단하십시오.
